Acoustic Source Localization (STM32 TDOA)

This project implements a real-time sound localization system using an array of 3 MEMS microphones and an STM32F446RE. It uses Time Difference of Arrival (TDOA) and Cross-Correlation to calculate the angle of incoming sound with sub-millisecond precision.
...
How to Run
    ...
    Open SoundLoc.ioc if you need to modify the pinout.

    Build and Flash to a Nucleo-F446RE.

    Open a Serial Monitor at 115200 baud (via SWV or UART depending on your config) to see the angle output.
